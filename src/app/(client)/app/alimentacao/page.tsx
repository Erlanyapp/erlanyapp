...
export default async function FoodPage() {
  ...
  try {
    ...
    return <div><PageHeader title="Alimentação" /><Hero eyebrow="CUIDAR TAMBÉM É NUTRIR" title="Escolhas que fazem bem" description="Orientações simples para uma rotina mais leve." icon="◒" className="food-hero" /><TabNavigation tabs={["Planos", "Receitas", "Orientações"]} selected="Planos" /><div className="food-list">{sections.length ? sections.map(({ plan, meals }) => <section className="food-plan" key={plan.id}><FoodCard title={plan.name} description={plan.description ?? "Plano alimentar disponível para você."} icon="◒" />{meals.length ? <div className="meal-list">{meals.map((meal) => <div className="card meal-row" key={meal.id}><div><strong>{meal.mealOrder}. {meal.name}</strong>{meal.guidance && <p>{meal.guidance}</p>}</div></div>)}</div> : <p className="muted food-empty-note">As refeições deste plano ainda não foram liberadas.</p>}</section>) : <EmptyState title="Nenhum plano disponível" description="Seu conteúdo de alimentação aparecerá aqui quando for liberado." />}{recipes.length > 0 && <section className="food-plan"><h2 className="subsection-title">Receitas</h2><div className="meal-list">{recipes.map((recipe) => <div className="card meal-row" key={recipe.id}><strong>{recipe.name}</strong>{recipe.description && <p>{recipe.description}</p>}{recipe.ingredients.length > 0 && <small>{recipe.ingredients.map((ingredient) => `${ingredient.name}${ingredient.quantity ? ` (${ingredient.quantity})` : ""}`).join(" · ")}</small>}{recipe.instructions && <p>{recipe.instructions}</p>}</div>)}</div></section>}</div></div>;
