...
export default async function WorkoutDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Keep the client-facing error generic while preserving the actual
    // Supabase/RLS failure in server logs for diagnosis.
    console.error("client.workout.detail_load_failed", { workoutId: id, error });
    return <div><PageHeader title="Treino" back backHref="/app/treinos" /><ContentError label="este treino" /></div>;
  }
}
