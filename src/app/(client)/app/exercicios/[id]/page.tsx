...
export default async function ExerciseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return <div><PageHeader title={exercise.name} back />{exercise.videoId && <VideoPlayer videoId={exercise.videoId} title={exercise.name} />}<p className="muted">{exercise.description ?? "Prepare-se para evoluir com segurança."}</p><Card><p className="eyebrow">INSTRUÇÕES</p><p>{exercise.instructions ?? "Siga a orientação do seu profissional."}</p><small>{exercise.muscles.length ? `Músculos: ${exercise.muscles.join(", ")}` : ""}</small></Card><Link className="button button-primary full-width exercise-cta" href="/app/treinos">Voltar aos treinos</Link></div>;
